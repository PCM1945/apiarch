...
class Worker:
    async def porcess_message(self, message: aiopika.abc.AbstractIncomingMessage):
        ...
        try:
            await message.ack() #sends an ACK flag to rabbit to unqueue the message after processing
        except Exception as e:
            await message.reject(requeue=True) #if the process fail reject and requeue request
